...
@section('content')
    ...
    <style>
        ...
        .row-line.discount span:last-child {
            color: #1e7a4a;
        }
            ...
            <div class="col-12 col-lg-5">
                <div class="cart-summary d-none d-lg-block">
                    ...
                    <div class="coupon-box mt-3" id="couponBoxDesktop" data-variant="desktop">
                        <div id="couponFormStateDesktop">
                            <div class="coupon-row">
                                <input type="text" class="coupon-input coupon-code-input"
                        ...
                        <div id="couponAppliedStateDesktop" style="display:none;">
                            <div class="coupon-applied-row">
                                ...
                                <button type="button" class="coupon-remove-btn coupon-remove-trigger"
                                    title="Remove coupon">
                    ...
                    <div class="row-line mt-3">
                        ...
                        <span style="font-size:14px;"
                            data-total="subtotalExGst">₹{{ number_format($totalExGst, 2) }}</span>
                    </div>
                    ...
                    <div class="row-line">
                        ...
                        <span style="font-size:14px;"
                            data-total="shipping">₹{{ number_format($totalShipping, 2) }}</span>
                    </div>

                    <div style="border-top:1px solid var(--line-strong);margin:8px 0;"></div>

                    <div class="row-line total">
                        <span>Total <span style="font-size:11px;font-weight:400;color:var(--soft);">(incl.
                                GST)</span></span>
                    ...
                    @if ($totalGst > 0)
                        <p
                            style="font-size:11px;color:var(--soft-2);text-align:right;margin-top:4px;letter-spacing:.04em;">
                ...
                <div class="d-lg-none" style="border:1px solid var(--line);background:var(--surface);">
                    <button type="button"
                        ...
                        <span>
                            ...
                            <strong style="margin-left:8px;"
                                id="mobileSummaryTotalLabel">₹{{ number_format($total + $totalShipping, 2) }}</strong>
                        </span>
                    ...
                    <div style="display:none;border-top:1px solid var(--line);padding:12px 16px;">
                        ...
                        {{-- Discount row (mobile) --}}
                        <div style="display:none;justify-content:space-between;padding:4px 0;font-size:13px;color:#1e7a4a;"
                            id="discountRowMobile">
            ...
            <div style="line-height:1.25;">
                ...
                @if ($totalGst > 0)
                    <div style="font-size:11px;color:var(--soft);">
                        ...
                        @endif
                        @if ($totalShipping > 0)
                            ₹{{ number_format($totalShipping, 2) }} ship
                        @endif
                    </div>
                @endif
            </div>
    ...
    <div class="coupon-modal-overlay" id="couponModalOverlay">
        <div class="coupon-modal">
            <div class="coupon-modal-header">
                ...
                <button type="button" class="coupon-modal-close" id="couponModalClose"><i
        ...
        (function() {
            ...
            function syncOtherBox(currentBoxId, data) {
                ...
                if (otherBox && otherBox.__setAppliedUI) {
                    const formStateId = otherId === 'couponBoxDesktop' ? 'couponFormStateDesktop' :
                        'couponFormStateMobile';
                    const appliedStateId = otherId === 'couponBoxDesktop' ? 'couponAppliedStateDesktop' :
                        'couponAppliedStateMobile';
                    ...
                    otherBox.querySelector('.applied-save-target').textContent = 'You saved ' + formatRupee(data
                        .discount_ex_gst);
                }
            }
            ...
            async function loadCoupons() {
                ...
                try {
                    const res = await fetch('{{ route('checkout.coupons.list') }}', {
                        headers: {
                            'Accept': 'application/json'
                        }
                    });
                    ...
                    if (!coupons.length) {
                        modalBody.innerHTML =
                            '<div class="coupon-modal-empty">No coupons available right now.</div>';
                        return;
                    }
                ...
                `).join('');
                    ...
                    modalBody.innerHTML =
                        '<div class="coupon-modal-empty">Could not load coupons. Please try again.</div>';
                }
            }
            ...
            @if (session('coupon'))
                (function restoreAppliedCoupon() {
                    ...
                    ['couponBoxDesktop', 'couponBoxMobile'].forEach(function(boxId) {
                        const box = document.getElementById(boxId);
                        if (!box) return;
                        const formStateId = boxId === 'couponBoxDesktop' ? 'couponFormStateDesktop' :
                            'couponFormStateMobile';
                        const appliedStateId = boxId === 'couponBoxDesktop' ? 'couponAppliedStateDesktop' :
                            'couponAppliedStateMobile';
                        document.getElementById(formStateId).style.display = 'none';
                        document.getElementById(appliedStateId).style.display = 'block';
                        box.classList.add('applied');
                        box.querySelector('.applied-code-target').textContent = data.code;
                        box.querySelector('.applied-desc-target').textContent = data.description || '';
                        box.querySelector('.applied-save-target').textContent = 'You saved ' + formatRupee(
                            data.discount_ex_gst);
                    });
