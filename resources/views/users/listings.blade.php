        ...
        @foreach(ucategories() as $cat)
            ...
            <li>
                <a href="{{ route('listing', $cat) }}"
                   class="lst-drawer-link {{ $isActive ? 'active' : '' }}">
                    <span>{{ $cat->name }}</span>
                    @if($isActive)
                            ...
                            @foreach($products as $product)
                                <div class="lst-card">
                                    ...
                                    <div class="lst-card-body">
                                        ...
                                        @if($product->size || $product->color)
                                            <div class="lst-card-attrs">
                                                ...
                                                @if($product->size && $product->color)<span class="lst-sep">·</span>@endif
                                    ...
                                    <div class="lst-card-foot">
                                        ...
                                        @if($product->stock <= 0)
                                            <span class="lst-action-btn lst-action-oos">
                                                <i class="bi bi-slash-circle"></i>
                                                <span>Out of Stock</span>
                                            </span>
                                        @else
                                            <form action="{{ route('cart.add') }}" method="POST" class="lst-action-form">
                                                @csrf
...
.pd-crumbs a { color: var(--soft); }
.pd-crumbs a:hover { color: var(--accent-2); }
.pd-crumbs > span { color: var(--soft-2); }
.pd-crumb-active { color: var(--ink); font-weight: 500; }

/* ── Body ── */
.lst-body { background: var(--bg); padding: 28px 0 72px; }
...
.lst-sidebar-link:hover { background: var(--surface); color: var(--accent-2); }
...
.lst-count { font-size: 12px; color: var(--soft); }
.lst-count strong { color: var(--ink); font-weight: 600; }
.lst-cat-btn {
...
.lst-grid {
    ...
    gap: 14px;      /* ← breathing room on mobile */
}
@media (min-width: 576px) {
    .lst-grid { grid-template-columns: repeat(2, 1fr); gap: 16px; }
}
@media (min-width: 768px) {
    .lst-grid { grid-template-columns: repeat(3, 1fr); gap: 18px; }
}
@media (min-width: 1200px) {
    .lst-grid { grid-template-columns: repeat(4, 1fr); gap: 20px; }
}
...
.lst-card-name:hover { color: var(--accent-2); }
.lst-card-attrs {
    display: flex; align-items: center; gap: 4px;
    font-size: 11px; color: var(--soft-2);
}
.lst-sep { color: var(--line-strong); }
.lst-card-price {
...
.lst-action-form { flex: 1; display: flex; }
.lst-action-btn {
    flex: 1;
...
.lst-action-form:last-child .lst-action-btn { border-right: none; }
.lst-action-btn:hover { color: var(--ink); background: var(--surface); }
.lst-action-btn i { font-size: 12px; }
.lst-action-oos {
...
.lst-action-cart:hover { color: var(--accent-2); background: rgba(176,141,87,0.07); }
...
.lst-empty-icon { font-size: 1.8rem; color: var(--line-strong); display: block; margin-bottom: 12px; }
.lst-empty-title {
    font-family: var(--font-display); font-weight: 500;
    color: var(--ink); margin-bottom: 6px;
}
.lst-empty-text { color: var(--soft); font-size: 13px; margin: 0; }
...
.lst-overlay.open { display: block; }
...
.lst-drawer-close:hover { background: var(--ink); color: var(--bg); }
...
.lst-drawer-link:hover { background: var(--surface); }
.lst-drawer-link.active {
    font-weight: 600;
    color: var(--accent-2);
    background: var(--accent-soft);
}
.lst-drawer-list li:last-child .lst-drawer-link { border-bottom: none; }
...
@media (max-width: 575.98px) {
    .lst-card-body { padding: 8px 10px 6px; }
    .lst-card-name { font-size: 12.5px; }
    .lst-price { font-size: 13px; }
    .lst-action-btn { font-size: 11px; padding: 8px 2px; gap: 3px; }
    .lst-action-btn i { font-size: 11px; }
    .lst-action-btn span { display: none; }   /* icon-only on very small screens */
    .lst-action-oos span { display: none; }
    .lst-action-oos i { display: inline; }
}

/* Show label again from 380px */
@media (min-width: 380px) {
    .lst-action-btn span { display: inline; }
    .lst-action-oos span { display: inline; }
}
</style>
