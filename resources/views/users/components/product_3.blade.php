    ...
    <div class="card product-item border-0 mb-4 h-100">
        ...
        <div class="card-body text-center py-3 px-2">
            ...
            <div class="d-flex justify-content-center align-items-center mb-2">
                ...
                @if($product->mrp && $product->mrp > $product->selling)
                    <h6 class="mb-0 text-muted ms-2"><del>₹{{ $product->mrp }}</del></h6>
                @endif
            </div>

            @if($product->size || $product->color)
                <div class="d-flex justify-content-center text-muted small mb-2">
                    @if($product->size)
        ...
        <div class="card-footer bg-light d-flex justify-content-between align-items-center px-3 py-1 border-top">
            ...
            <form action="{{ route('cart.add') }}" method="POST" class="mb-0">
                ...
                <button type="submit" class="btn btn-sm text-dark d-flex align-items-center {{ $product->stock <= 0 ? 'disabled' : '' }}">
